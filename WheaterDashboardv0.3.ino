...
// Fetch weather + AQI using the SAME endpoints as the OLED version
bool fetchWeatherAndAQI() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("No WiFi, cannot fetch weather");
    return false;
  }

  WiFiClientSecure client;
  client.setInsecure();  // ignore TLS cert

  // ---------- WEATHER (same as OLED) ----------
  HTTPClient https;
  ...
  Serial.println("Fetching weather (OLED-style)...");
  ...
  if (current.isNull()) {
    Serial.println("  current object missing");
    return false;
  }

  float tempC   = current["temperature_2m"]       | 0.0f;
  float rh      = current["relative_humidity_2m"] | 0.0f;
  int   wcode   = current["weather_code"]         | 0;
  float wind_ms = current["wind_speed_10m"]       | 0.0f;
  float uv      = current["uv_index"]             | 0.0f;
  float vis_m   = current["visibility"]           | 0.0f;

  // Convert to match TFT labels
  float wind_kmh = wind_ms * 3.6f;
  float vis_km   = vis_m / 1000.0f;

  // ---------- AQI (European, same as OLED) ----------
  HTTPClient httpsAQ;
  String urlAQ = String("https://air-quality-api.open-meteo.com/v1/air-quality?")
                 + "latitude="   + String(LATITUDE, 4)
                 + "&longitude=" + String(LONGITUDE, 4)
                 + "&hourly=european_aqi"
                 + "&timezone=auto&forecast_days=1";

  Serial.println("Fetching AQI (EU)...");
  int aqiVal = 0;

  if (httpsAQ.begin(client, urlAQ)) {
    int httpCode2 = httpsAQ.GET();
    if (httpCode2 == HTTP_CODE_OK) {
      String payload2 = httpsAQ.getString();
      StaticJsonDocument<4096> doc2;
      DeserializationError err2 = deserializeJson(doc2, payload2);
      if (!err2) {
        JsonArray aqiArr = doc2["hourly"]["european_aqi"];
        if (!aqiArr.isNull() && aqiArr.size() > 0) {
          aqiVal = aqiArr[0] | 0;
        }
      } else {
        Serial.print("  JSON AQI error: ");
    ...
    Serial.println("  begin(AQI) failed");
  }

  // ---------- Fill targetData (same semantics as OLED) ----------
  targetData.temp     = tempC;
  targetData.humidity = (int)rh;
  targetData.cond     = weatherCodeToText(wcode);
  targetData.wind     = wind_kmh;   // km/h
  targetData.uv       = uv;
  targetData.vis      = vis_km;     // km
  targetData.aqi      = aqiVal;
  targetData.location = LOCATION_NAME;
  targetData.age      = 0;          // "Updated" timer reset

  Serial.println("Weather + AQI updated OK (TFT aligned with OLED)");
  return true;
}
